...
- **순수 HTML / CSS / JS** — 프레임워크·빌드 도구 없이 브라우저에서 바로 동작
- **데이터 저장**: `localStorage` (메뉴 데이터, 장바구니, 주문 내역 모두 브라우저에 저장)
- **페이지 간 이동**: 일반 `<a href>` 링크 + 쿼리스트링(`?id=`)으로 상세/수정 대상 지정
- **공통 로직**: `js/data.js`(메뉴 시드 데이터 및 초기화), `js/utils.js`(포맷팅, localStorage 헬퍼, 장바구니 계산 등)를 각 페이지에서 `<script>` 태그로 로드

## 🗂 데이터 모델 (localStorage 키)

```
cafe:menus     → [{ id, category, name, description, price, imageUrl, soldOut, options: [{ id, name, type, required, choices: [{ id, name, extraPrice }] }] }]
cafe:basket    → [{ menuId, name, price, quantity, selectedOptions: [{ groupName, choiceName, extraPrice }] }]
cafe:orders    → [{ id, orderNo, items, totalPrice, customerName, phone, pickupTime, note, status, createdAt }]
```
- `status`: `RECEIVED` → `PREPARING` → `READY` → `COMPLETED` (+ `CANCELLED`)
- 앱 최초 로드 시 `cafe:menus`가 없으면 `js/data.js`의 시드 데이터로 초기화
